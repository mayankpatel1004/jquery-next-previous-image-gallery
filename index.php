...
$IMAGEEXTENSION = array("gif","png","jpg","jpeg");
$err_msg = '';
$img_directory = 'thumb/square';
?>
<!doctype html>
<html>
<head>
...
<title>Image Thumbnail Demo</title>
</head>
...
<div id="slide_images">
	<?php
		$cnt=0;
		$img_arr = array();
		foreach(glob($img_directory.'/*.*') as $filename){
     		//echo $filename.'<br>';
			$pos = strpos($filename, '/t_');
			if($pos) { //echo $filename.'<br>'; 
				$cnt++;
				$popup_img = str_replace('/t_','/',$filename);
				$img_arr[$cnt] = $popup_img;
			?>
            <script> 
				img_array[<?php echo $cnt;?>] = '<?php echo $img_arr[$cnt];?>';
            ...
            <div style="width:150px; height:150px; border:thick solid #666666; overflow:hidden; position:relative; float:left; margin-right:10px; margin-top:5px;">
                <a onClick="setImage('<?php echo $popup_img;?>','<?php echo $cnt;?>');" class="vbox" style="cursor:pointer;">
                	<img id="img_<?php echo $cnt;?>" src="<?php echo $filename;?>" border="0" style="position: absolute;" onload="OnImageLoad(event);" />
                </a>
            </div>
<?php			//if($cnt%8==0) echo '<br>';
			}
 		}
